...
![图1.2 信号的时钟周期和 10%~90% 上升时间](images/Ch01_fig1.2_waveform.png)

一个数字信号波形，标注了时钟周期 $T_{clock}$
和 10%~90% 上升时间 $T_r$ 。
...
![图1.3 在原理图上标注各支路电源的电流值](images/Ch01_fig1.3_current_label.png)



### (3) 标注关键电流通路
在原理图上标注大电流路径(如 MOSFET 源极-漏极通路)，提示 PCB 工程师做加宽走线、增加过孔等处理。例如 3.3V 电源主通路电流达 5A，应在原理图上标注 "3.3V_SRC 与 3.3V 为电流关键路径"。

![图1.4 在原理图上标注关键电流通路](images/Ch01_fig1.4_current_path.png)
...
![图1.5 原理图的绘制应兼顾PCB上器件的布放要求](images/Ch01_fig1.5_placement.png)

### (5) 电容滤波电路按 PCB 排列顺序绘制
PCB 上 1μF 在最外边，100μF ~ 10μF ~ 0.1μF，原理图也应同序。

![图1.6 原理图兼顾电容的布放顺序](images/Ch01_fig1.6_capacitor_order.png)
